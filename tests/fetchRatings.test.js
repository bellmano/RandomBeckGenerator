...
describe("fetchRatings.js", () => {
  ...
  test("extractTitleId returns the IMDb title id when present", () => {
    expect(extractTitleId("https://imdb.com/title/tt0118693")).toBe(
      "tt0118693",
    );
  });
  ...
  test("parseMovies throws when the database export is not an array", () => {
    expect(() => parseMovies({ title: "Movie" })).toThrow(
      "beckDB.js must export an array of movies.",
    );
  });
    ...
    const fetchImpl = jest
      ...
      .mockResolvedValue(
        createDatasetResponse(
          ["tconst\taverageRating\tnumVotes", "tt0118693\t6.2\t1952"].join(
            "\n",
          ),
        ),
      );
  ...
  test("fetchRatingsMap throws when fetch is unavailable", async () => {
    ...
    await expect(fetchRatingsMap(new Set(["tt1"]))).rejects.toThrow(
  ...
  test("fetchRatingsMap throws when the dataset request fails", async () => {
    ...
    await expect(fetchRatingsMap(new Set(["tt1"]), fetchImpl)).rejects.toThrow(
      "Failed to fetch IMDb ratings dataset: 503 Service Unavailable",
    );
  });
  ...
  test("updateRatings handles the complete flow with mixed movie states", async () => {
    ...
    const fetchImpl = jest
      ...
      .mockResolvedValue(
        createDatasetResponse(
          ["tconst\taverageRating\tnumVotes", "tt1234567\t7.5\t1200"].join(
            "\n",
          ),
        ),
      );
    ...
    expect(fs.writeFileSync).toHaveBeenCalledWith(
      expect.any(String),
      expect.stringContaining('imdbRating: "7.5"'),
      "utf8",
    );
    expect(fs.writeFileSync).toHaveBeenCalledWith(
      expect.any(String),
      expect.stringContaining("module.exports = beckMovies;"),
      "utf8",
    );
    expect(consoleLogSpy).toHaveBeenCalledWith(
      "Fetching latest IMDb ratings dataset...",
    );
    expect(consoleLogSpy).toHaveBeenCalledWith(
      "Updating IMDb rating for: Rated Movie",
    );
    expect(consoleLogSpy).toHaveBeenCalledWith("  -> Rating: 7.5");
    expect(consoleLogSpy).toHaveBeenCalledWith(
      "Updating IMDb rating for: Missing Rating",
    );
    expect(consoleLogSpy).toHaveBeenCalledWith("  -> Rating not found");
    expect(consoleLogSpy).toHaveBeenCalledWith(
      "Invalid imdbUrl for: Invalid URL",
    );
    expect(consoleLogSpy).toHaveBeenCalledWith("No imdbUrl for: No URL");
    expect(consoleLogSpy).toHaveBeenCalledWith(
      "beckDB.js updated with latest IMDb ratings.",
    );
  });
  ...
  test("updateRatings returns null when fetching the dataset fails", async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      "Failed to fetch IMDb ratings dataset:",
      expect.any(Error),
    );
  });
  ...
  test("updateRatings supports injected movie loading and default global fetch", async () => {
    const loadMovies = jest.fn().mockReturnValue([
      {
        ...
        title: "Movie",
        year: 2020,
        imdbUrl: "https://imdb.com/title/tt42",
      },
    ]);
    ...
    globalThis.fetch = jest
      ...
      .mockResolvedValue(
        createDatasetResponse(
          ["tconst\taverageRating\tnumVotes", "tt42\t8.4\t4200"].join("\n"),
        ),
      );
  ...
  test("main delegates to updateRatings", async () => {
    const loadMovies = jest.fn().mockReturnValue([
      {
        ...
        title: "Movie",
        year: 2020,
        imdbUrl: "https://imdb.com/title/tt99",
      },
    ]);
    const writeFileSync = jest.fn();
    const originalFetch = globalThis.fetch;
    globalThis.fetch = jest
      ...
      .mockResolvedValue(
        createDatasetResponse(
          ["tconst\taverageRating\tnumVotes", "tt99\t6.8\t680"].join("\n"),
        ),
      );
    ...
    expect(writeFileSync).toHaveBeenCalledWith(
      expect.any(String),
      expect.stringContaining('imdbRating: "6.8"'),
      "utf8",
    );
  ...
  test("runMain delegates to main when no unexpected error occurs", async () => {
    const loadMovies = jest.fn().mockReturnValue([
      {
        ...
        title: "Movie",
        year: 2020,
        imdbUrl: "https://imdb.com/title/tt100",
      },
    ]);
    const writeFileSync = jest.fn();
    const originalFetch = globalThis.fetch;
    globalThis.fetch = jest
      ...
      .mockResolvedValue(
        createDatasetResponse(
          ["tconst\taverageRating\tnumVotes", "tt100\t7.1\t710"].join("\n"),
        ),
      );
    ...
    expect(writeFileSync).toHaveBeenCalledWith(
      expect.any(String),
      expect.stringContaining('imdbRating: "7.1"'),
      "utf8",
    );
    expect(consoleErrorSpy).not.toHaveBeenCalledWith(
      "Unexpected error while updating ratings:",
      expect.any(Error),
    );
  ...
  test("runMain reports unexpected errors and sets exitCode", async () => {
    ...
    await runMain({
      loadMovies: jest.fn().mockReturnValue([
        {
          number: 1,
          title: "Movie",
          year: 2020,
          imdbUrl: "https://imdb.com/title/tt101",
        },
      ]),
      ...
      fetchImpl: jest
        ...
        .mockResolvedValue(
          createDatasetResponse(
            ["tconst\taverageRating\tnumVotes", "tt101\t7.9\t790"].join("\n"),
          ),
        ),
    });
